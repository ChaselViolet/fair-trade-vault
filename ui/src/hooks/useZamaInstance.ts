...
import { initializeFHEVM, encryptInput, resetFHEVMInstance, decryptEuint32, decryptEbool } from '../lib/fhevm';
...
export function useZamaInstance() {
  ...
  const encrypt = useCallback(
    async (contractAddress: string, userAddress: string, value: number) => {
      ...
      return encryptInput(instance, contractAddress, userAddress, value);
    },
    [instance]
  );

  // Decryption function for euint32
  const decryptEuint = useCallback(
  ...
  const decryptBool = useCallback(
    async (handle: string, contractAddress: string, userAddress: string) => {
      ...
      return decryptEbool(instance, handle, contractAddress, userAddress, signer, chainId);
    },
    [instance, chainId]
  );
